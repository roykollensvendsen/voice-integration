...
**Result: 14 of 15 rules turned off the test that names them.** None survived. Killed only by other tests: a delegation waits for the work rather than reading back a receipt.
...
| a path argument goes into the URL, never into the body | 11 | `test_a_path_argument_goes_into_the_url_never_into_the_body` | yes |
...
| a delegation waits for the work rather than reading back a receipt | 6 | `test_a_delegation_waits_for_the_work_rather_than_reading_back_a_receipt` | NO |
| a voice turn asks the gateway to finish inside it | 6 | `test_a_voice_turn_asks_the_gateway_to_finish_inside_it` | yes |
| only a word that is plainly yes or no answers a permission question | 6 | `test_only_a_word_that_is_plainly_yes_or_no_answers_a_permission_question` | yes |
